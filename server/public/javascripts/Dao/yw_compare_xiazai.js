...
let getYwCompareXiazai = async function () {
    ...
    let result2 = await new Promise((resolve, reject) => {
        let sqlStr = "SELECT name FROM yw_compare_xiazai";
    ...
    let result1 = await new Promise((resolve, reject) => {
        let sqlStr = "SELECT xiazai FROM yw_compare_xiazai";
        conn.query(sqlStr, function (err, ret) {
            if (err) {
                // 回滚之前的数据库操作, 直至碰到 beginTransaction
                return conn.rollback(() => {
    ...
    for(let i=0;i<result2.length;i++){
        result.push({name:result2[i].name,bili:result1[i].xiazai})
    }
    return result;
};
